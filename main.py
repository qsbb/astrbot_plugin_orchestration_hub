...
class OrchestrationHubPlugin(Star):
    ...
    def __init__(self, context: Context, config: Any = None) -> None:
        ...
        self.registry = (
            ServiceRegistry()
        )  # default lease is overridden by explicit registration lease when configured
        ...
        self.invocation = InvocationEngine(
            ...
            self.policy,
            self.telemetry,
            max_inflight=int(self._config("GLOBAL_MAX_INFLIGHT", 128)),
            max_inline_payload_bytes=int(
                self._config("MAX_INLINE_PAYLOAD_BYTES", 262144)
            ),
        )
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            "[orchestration-hub] v%s loaded; pages=%s",
            __version__,
            self.page_registered,
        )
    ...
    async def _lease_supervisor(self) -> None:
        try:
            while not self._terminated:
                await asyncio.sleep(
                    max(1.0, min(10.0, float(self._config("LEASE_SWEEP_SECONDS", 5))))
                )
    ...
    async def hub_services(self, event):
        ...
        lines = ["已注册服务"]
        lines.extend(
            f"- {item.descriptor.service} {item.descriptor.version} [{item.state}] {item.token.instance_id}"
    ...
    async def hub_diagnose(self, event):
        report = {
            "pages": self.pages.adapter.report()
            if self.pages.adapter
            else {"available": False, "degraded": True},
            "registry_accepting": not self._terminated,
        }
        yield event.plain_result(
    ...
    async def terminate(self) -> None:
        ...
        await self.store.append_audit(
            {"action": "terminate", "result": "ok", "revision": self.registry.revision}
        )
